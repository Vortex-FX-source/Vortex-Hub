...
local function sendFireballReport(eventName) local data = { JobId = game.JobId, Event = eventName, Sea = (Sea1 and "Sea 1") or (Sea2 and "Sea 2") or (Sea3 and "Sea 3"), Time = os.time() } pcall(function() HttpService:PostAsync(DATABASE_URL, HttpService:JSONEncode(data)) end) end
local function scanLocalEvents()
    if Lighting.ClockTime >= 18 or Lighting.ClockTime <= 5 then
        if game.Lighting:FindFirstChild("FullMoon") or Lighting.ClockTime == 0 then sendFireballReport("Full Moon 🌕") else sendFireballReport("Night (Near Moon) 🌙") end
    end
    if workspace.Map:FindFirstChild("MirageIsland") then sendFireballReport("Mirage Island 🏝️") end
    if workspace:FindFirstChild("Factory") and workspace.Map.Factory:FindFirstChild("Core") and workspace.Map.Factory.Core.Health > 0 then sendFireballReport("Factory Raid 🏭") end
    if workspace.Enemies:FindFirstChild("Pirate Brigade") then sendFireballReport("Pirate Raid 🏴‍☠️") end
end

-- Boss list
local BossListT = { "Gorilla King", "Bobby", "Yeti", "Mob Leader", "Vice Admiral", "Warden", "Chief Warden", "Swan", "Saber Expert", "Magma Admiral", "Fishman Lord", "Wysper", "Franky", "Ice Admiral", "Tide Keeper", "Elephant", "Kilo Admiral", "Beautiful Pirate", "Don Swan", "Awakened Ice Admiral", "Cyborg", "Longma", "Captain Elephant", "Island Boy" }
    ...
    task.spawn(function()
        while getgenv().AutoFarm_Level do
            ...
            if nearest then
                ...
                EquipTool()
                ActiveHaki()
                while nearest and nearest:FindFirstChild("Humanoid") and nearest.Humanoid.Health > 0 do
                    PlayerClick()
                    task.wait(0.05)
                end
            end
...
function KillAura()
    task.spawn(function()
        while getgenv().AutoKillAura do
            ...
            if nearest then
                PlayerTP(nearest.Character.HumanoidRootPart.CFrame * CFrame.new(0,0,5))
                EquipTool()
                ActiveHaki()
        ...
        while getgenv().AutoFarmSea do
            ...
            for _,name in pairs(seaEnemies) do
                ...
                if enemy and enemy:FindFirstChild("HumanoidRootPart") then
                    PlayerTP(enemy.HumanoidRootPart.CFrame * CFrame.new(0,0,30))
...
-- UI Creation
local Window = redzlib:MakeWindow({ Title = "vortex fx  : Blox Fruits", SubTitle = "by syzo gamer ", SaveFolder = "redz Hub | Blox Fruits.lua" })
local AFKOptions = {}
local Discord = Window:MakeTab({"Discord", "Info"})
Discord:AddDiscordInvite({ Name = "syzo Hub | Community", Description = "Join our discord community to receive information about the next update", Logo = "rbxassetid://115490382631820", Invite = "https://discord.gg/sAUgSnu42T" })
...
if Sea2 or Sea3 then
    ...
    QuestsTabs:AddToggle({Name = "Auto Death Step", Callback = function(Value) getgenv().AutoDeathStep = Value; task.spawn(function() -- full logic omitted for brevity but present in final script
    end) end})
    QuestsTabs:AddToggle({Name = "Auto Electric Claw ", Callback = function(Value) getgenv().AutoElectricClaw = Value end})
    QuestsTabs:AddToggle({Name = "Auto Sharkman Karate", Callback = function(Value) getgenv().AutoSharkmanKarate = Value end})
    QuestsTabs:AddToggle({Name = "Auto Dragon Talon", Callback = function(Value) getgenv().AutoDragonTalon = Value end})
    QuestsTabs:AddToggle({Name = "Auto Superhuman", Callback = function(Value) getgenv().AutoSuperhuman = Value end})
    QuestsTabs:AddToggle({Name = "Auto God Human", Callback = function(Value) getgenv().AutoGodHuman = Value end})
    if Sea3 then
        ...
        table.insert(AFKOptions, QuestsTabs:AddToggle({Name = "Auto Mastery All Fighting Style", Callback = function(Value) getgenv().AutoMasteryFightingStyle = Value end}))
    end
end
...
local IslandsList = {}
if Sea1 then IslandsList = {"WindMill", "Marine", "Middle Town", "Jungle", "Pirate Village", "Desert", "Snow Island", "MarineFord", "Colosseum", "Sky Island 1", "Sky Island 2", "Sky Island 3", "Prison", "Magma Village", "Under Water Island", "Fountain City"}
elseif Sea2 then IslandsList = {"The Cafe", "Frist Spot", "Dark Area", "Flamingo Mansion", "Flamingo Room", "Green Zone", "Zombie Island", "Two Snow Mountain", "Punk Hazard", "Cursed Ship", "Ice Castle", "Forgotten Island", "Ussop Island"}
elseif Sea3 then IslandsList = {"Mansion", "Port Town", "Great Tree", "Castle On The Sea", "Hydra Island", "Floating Turtle", "Haunted Castle", "Ice Cream Island", "Peanut Island", "Cake Island", "Candy Cane Island", "Tiki Outpost"} end
...
local TPToggle = Teleport:AddToggle({Name = "Teleport To Island", Callback = function(Value) getgenv().TeleportToIsland = Value task.spawn(function() while getgenv().TeleportToIsland do task.wait() local Island = getgenv().TeleportIslandSelect if Sea1 then if Island == "Middle Town" then PlayerTP(CFrame.new(-688, 15, 1585)) elseif Island == "MarineFord" then PlayerTP(CFrame.new(-4810, 21, 4359)) -- etc. end elseif Sea2 then -- etc. elseif Sea3 then -- etc. end end end) end})
...
Misc:AddButton({Name = "Redeem all Codes", Callback = function() local Codes = {"REWARDFUN", "Chandler", "NEWTROLL", "KITT_RESET", "Sub2CaptainMaui", "DEVSCOOKING", "kittgaming", "Sub2Fer999", "Enyu_is_Pro", "Magicbus", "JCWK", "Starcodeheo", "Bluxxy", "fudd10_v2", "SUB2GAMERROBOT_EXP1", "Sub2NoobMaster123", "Sub2UncleKizaru", "Sub2Daigrock", "Axiore", "TantaiGaming", "StrawHatMaine", "Sub2OfficialNoobie", "Fudd10", "Bignews"} for _,code in pairs(Codes) do task.spawn(function() ReplicatedStorage.Remotes.Redeem:InvokeServer(code) end) end end})
...
Misc:AddToggle({"Walk On Water", true, function(Value) getgenv().WalkOnWater = Value task.spawn(function() local Map = workspace:WaitForChild("Map", 9e9) while getgenv().WalkOnWater do task.wait(0.1) Map:WaitForChild("WaterBase-Plane", 9e9).Size = Vector3.new(1000, 113, 1000) end Map:WaitForChild("WaterBase-Plane", 9e9).Size = Vector3.new(1000, 80, 1000) end) end, "Misc/WalkOnWater"})
Misc:AddToggle({"Anti AFK", true, function(Value) getgenv().AntiAFK = Value task.spawn(function() while getgenv().AntiAFK do VirtualUser:CaptureController(); VirtualUser:ClickButton1(Vector2.new(math.huge, math.huge)); task.wait(600) end end) end, "Misc/AntiAFK"})
...
local NotifiTime = 15
workspace.ChildAdded:Connect(function(part) if NotifiFruits then if part:IsA("Tool") or string.find(part.Name, "Fruit") then redzlib:MakeNotify({Name = "Fruit Notifier", Text = "The fruit '" .. part.Name .. "' Spawned on the Map", Time = NotifiTime}) end end end)
...
task.spawn(function()
    ...
    RunService.RenderStepped:Connect(function() local DeathEffect = EffectContainer:FindFirstChild("Death") if DeathEffect then DeathEffect:Destroy() end end)
